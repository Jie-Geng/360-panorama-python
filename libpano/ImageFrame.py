...
class ImageFrame:
    ...
    def preprocess_image(self, scale, metrics, temp_folder):
        self.load_image()
        print('scale={}, ({},{})'.format(scale, self.width, self.height))
        ...
        if Config.order_warp_first:
            ...
            # if self.width > 2000:
            #     x = int((self.width - 2000) / 2)
            #     self.crop_rect(x, 0, 2000, self.height)
        else:
        ...
        # TODO: remove too overlapping areas
        #
        # erase_percent = 0.15
        # if self.row == 4:
        #     self.mask[:int(height * erase_percent), :] = 0
        # if self.row == 1:
        #     self.mask[int(height * (1-erase_percent)):, :] = 0
        # if self.row == 0:
        #     self.mask[int(height * (1-erase_percent/2)):, :] = 0
        ...
        print(os.path.join(temp_folder, file_name))
